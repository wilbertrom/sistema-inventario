...
<div class="container-fluid">
    ...
    <div class="page-header">
        <h2><i class="fas fa-tools" style="color:var(--rojo);margin-right:8px;"></i> Orden de Mantenimiento #<?php echo $orden->id; ?></h2>
        <div class="acciones">
            <a href="<?php echo base_url('orden-mantenimiento/pdf/'.$orden->id); ?>" class="btn-r" target="_blank">
            ...
            <form action="<?php echo base_url('orden-mantenimiento/actualizar/'.$orden->id); ?>" method="post" enctype="multipart/form-data">
                ...
                <div class="row">
                    ...
                    <div class="col-md-4">
                        <div class="f-group">
                            ...
                            <select name="solicitante" class="f-control">
                                <option value="">— Selecciona —</option>
                                <?php foreach($firmantes_resp as $f): ?>
                                <option value="<?php echo htmlspecialchars($f->nombre); ?>"
                                    <?php echo (isset($orden->solicitante) && $orden->solicitante == $f->nombre) ? 'selected' : ''; ?>>
                                    <?php echo htmlspecialchars($f->nombre); ?>
                                </option>
                                <?php endforeach; ?>
                                <?php
                                $nombres_sol = array_map(function($f){ return $f->nombre; }, (array)$firmantes_resp);
                                if(!empty($orden->solicitante) && !in_array($orden->solicitante, $nombres_sol)): ?>
                                <option value="<?php echo htmlspecialchars($orden->solicitante); ?>" selected>
                ...
                <div class="row">
                    ...
                    <div class="col-md-6">
                        ...
                        <div class="f-group">
                            <label class="f-label">Imagen de especificación
                ...
                <div class="row">
                    <div class="col-md-6">
                        <div class="f-group">
                            <label class="f-label">Nombre y firma de conformidad </label>
                            <select name="responsable_mantenimiento" class="f-control">
                                <option value="">— Selecciona —</option>
                                <?php foreach($firmantes_resp as $f): ?>
                                <option value="<?php echo htmlspecialchars($f->nombre); ?>"
                                    <?php echo (isset($orden->responsable_mantenimiento) && $orden->responsable_mantenimiento == $f->nombre) ? 'selected' : ''; ?>>
                                    <?php echo htmlspecialchars($f->nombre); ?>
                                </option>
                                <?php endforeach; ?>
                                <?php
                                $nombres_resp = array_map(function($f){ return $f->nombre; }, (array)$firmantes_resp);
                                if(!empty($orden->responsable_mantenimiento) && !in_array($orden->responsable_mantenimiento, $nombres_resp)): ?>
                                <option value="<?php echo htmlspecialchars($orden->responsable_mantenimiento); ?>" selected>
                                    <?php echo htmlspecialchars($orden->responsable_mantenimiento); ?>
                                </option>
                                <?php endif; ?>
                            </select>
                        </div>
                    ...
                    <div class="col-md-6">
                        <div class="f-group">
                            <label class="f-label">Verificado por nombre y firma </label>
                            <select name="verificado_por" class="f-control">
                                <option value="">— Selecciona —</option>
                                <?php foreach($firmantes_verif as $f): ?>
                                <option value="<?php echo htmlspecialchars($f->nombre); ?>"
                                    <?php echo (isset($orden->verificado_por) && $orden->verificado_por == $f->nombre) ? 'selected' : ''; ?>>
                                    <?php echo htmlspecialchars($f->nombre); ?>
                                </option>
                                <?php endforeach; ?>
                                <?php
                                $nombres_verif = array_map(function($f){ return $f->nombre; }, (array)$firmantes_verif);
                                if(!empty($orden->verificado_por) && !in_array($orden->verificado_por, $nombres_verif)): ?>
                                <option value="<?php echo htmlspecialchars($orden->verificado_por); ?>" selected>
                                    <?php echo htmlspecialchars($orden->verificado_por); ?>
                                </option>
                                <?php endif; ?>
                            </select>
                        </div>
                ...
                <div style="margin-bottom:16px;">
                    <button type="button" class="btn-r" style="padding:6px 14px;font-size:12px;"
        ...
        <div class="section-card-body">
            ...
            <div style="background:#f8fafc;border-radius:10px;padding:18px;border:1.5px dashed var(--gris-borde);">
                <p style="font-size:13px;font-weight:700;color:var(--texto-dark);margin-bottom:14px;">
                    <i class="fas fa-plus-circle" style="color:var(--azul);"></i> Agregar trabajo realizado
                </p>
                    ...
                    <div class="row">
                        <div class="col-md-2">
                            <div class="f-group">
                                ...
                                <input type="number" name="costo" class="f-control" step="0.01" min="0" value="0">
                            </div>
                        </div>
                        <div class="col-md-3">
                            <div class="f-group">
                                <label class="f-label">Fecha de realización</label>
                                <input type="date" name="fecha_realizacion" class="f-control" value="<?php echo date('Y-m-d'); ?>">
                            </div>
                        </div>
                        <div class="col-md-7">
                            <div class="f-group">
        ...
        <div class="modal-content" style="border-radius:14px;overflow:hidden;border:none;">
            <div class="modal-header" style="background:linear-gradient(135deg,#8B1A10,#a52119);border:none;padding:18px 22px;">
                <h5 class="modal-title" style="color:white;font-weight:700;">
                    <i class="fas fa-user-plus mr-2"></i> Agregar firmante
                </h5>
            ...
            <div class="modal-body" style="padding:22px;">
                <div class="f-group">
                    ...
                    <input type="text" id="nf_nombre" class="f-control" placeholder="Ej: Dr. Juan Pérez López">
                </div>
                <div class="f-group">
                    <label class="f-label">Cargo *</label>
                    <input type="text" id="nf_cargo" class="f-control" placeholder="Ej: Jefe de Laboratorio">
                </div>
                <div class="f-group">
                    <label class="f-label">Rol *</label>
                    <select id="nf_rol" class="f-control">
                        <option value="">— Selecciona —</option>
                        <option value="jefe_laboratorio">Nombre y firma de conformidad </option>
                        <option value="vo_bo">Verificado por</option>
                    </select>
                </div>
...
$('#btnGuardarFirmante').click(function() {
    ...
    $.ajax({
        ...
        success: function(r) {
            if (r.success) {
                var sel = (rol === 'jefe_laboratorio') ? '[name="responsable_mantenimiento"]' : '[name="verificado_por"]';
