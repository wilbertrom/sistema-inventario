...
.page-header {
    ...
    display: flex; align-items: center; border-left: 4px solid var(--rojo);
}
.page-header h2 {
...
.mesa-card {
    ...
    border: 1px solid var(--gris-borde); margin-bottom: 24px;
...
.mesa-header {
    ...
    color: white; padding: 16px 22px; display: flex; align-items: center; gap: 10px;
}
.mesa-header h4 { margin: 0; font-size: 16px; font-weight: 700; }
.mesa-header .mesa-num {
    background: white; color: var(--rojo); width: 32px; height: 32px;
    border-radius: 50%; display: flex; align-items: center; justify-content: center;
    font-weight: 700; font-size: 14px; flex-shrink: 0;
}
.mesa-header span { margin-left: auto; font-size: 12px; opacity: .9; }
...
.equipo-item .eq-label {
    font-size: 10px; color: var(--texto-mid); display: block;
}

/* ── Dropdown personalizado ── */
.dropdown-menu {
    border: none; box-shadow: 0 8px 24px rgba(0,0,0,.12);
    border-radius: 10px; border-top: 2px solid var(--rojo);
    padding: 6px 0; min-width: 140px;
}
.dropdown-item {
    font-size: 13px; padding: 9px 16px; color: var(--texto-mid);
    display: flex; align-items: center; gap: 8px;
    transition: background .15s, color .15s;
}
.dropdown-item:hover { background: var(--rojo-light); color: var(--rojo); }
.dropdown-item i.fa-link { color: var(--rojo); }
.dropdown-item i.fa-info-circle { color: #2563eb; } /* Se mantiene azul para diferenciar */
</style>
