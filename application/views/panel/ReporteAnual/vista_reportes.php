...
.page-header h2 {
    font-size: 20px; font-weight: 700; color: var(--texto-dark); margin: 0;
}
...
.btn-r:hover {
    box-shadow: 0 6px 18px rgba(165,33,25,.3); transform: translateY(-1px);
...
.reporte-card {
    ...
    border: 1px solid var(--gris-borde); overflow: hidden; transition: all .25s;
}
.reporte-card:hover {
    box-shadow: 0 12px 32px rgba(165,33,25,.12); transform: translateY(-3px);
}
...
.rc-top::after {
    ...
    width: 70px; height: 70px; background: rgba(255,255,255,.06); border-radius: 50%;
}
.rc-top::before {
...
.rc-year {
    font-size: 36px; font-weight: 700; color: white; display: block;
...
.mes-btn {
    display: block; padding: 7px 4px; background: #f8fafc;
    border: 1.5px solid var(--gris-borde); border-radius: 7px;
    text-align: center; font-size: 11px; font-weight: 700;
    color: var(--texto-mid); text-decoration: none; transition: all .15s;
    letter-spacing: .3px;
}
.mes-btn:hover {
    background: var(--rojo-light); border-color: var(--rojo);
    color: var(--rojo); text-decoration: none; transform: scale(1.04);
}
...
.rc-actions a, .rc-actions button {
    ...
    gap: 6px; padding: 9px; border-radius: var(--radio-sm);
...
.card-nuevo {
    background: white; border-radius: var(--radio); box-shadow: var(--sombra-sm);
    border: 2px dashed var(--gris-borde); overflow: hidden; transition: all .25s;
    display: flex; flex-direction: column; align-items: center;
    justify-content: center; min-height: 220px; cursor: pointer;
}
...
.cn-icon {
    ...
    color: var(--texto-mid); display: flex; align-items: center;
    justify-content: center; font-size: 22px; margin-bottom: 12px;
    transition: all .25s;
}
.cn-text {
    font-size: 14px; font-weight: 700; color: var(--texto-mid);
...
/* ── Alerts ── */
.alert-m {
    border-radius: var(--radio-sm); padding: 12px 18px; font-size: 13.5px;
    display: flex; align-items: center; gap: 10px; margin-bottom: 18px; border: none;
}
...
/* ── Estado vacío ── */
.empty-state {
    text-align: center; padding: 60px 20px; color: var(--texto-mid);
    grid-column: 1 / -1;
}
.empty-state i {
    font-size: 52px; color: #cbd5e1; display: block; margin-bottom: 14px;
}
</style>
...
<div class="container-fluid">
    ...
    <div class="page-header">
        ...
        <div style="display:flex;align-items:center;gap:12px;">
            ...
            <button class="btn-g" data-toggle="modal" data-target="#modalInfoLab"
                    style="padding:7px 14px;font-size:12px;">
                <i class="fas fa-cog"></i> Configurar info
            </button>
        </div>
    </div>
    ...
    <div class="modal-dialog modal-sm"><div class="modal-content" style="border-radius:14px;overflow:hidden;border:none;">
        <div class="modal-header" style="background:linear-gradient(135deg, var(--rojo-dark), var(--rojo)); border:none; padding:20px 24px;">
            <h5 class="modal-title" style="color:white;font-weight:700;font-size:16px;">
                <i class="fas fa-plus-circle mr-2" style="color:white;"></i> Nuevo Reporte
            </h5>
...
<!-- ── Modal: Configurar info laboratorio ─────────────────── -->
<div class="modal fade" id="modalInfoLab" tabindex="-1" role="dialog">
    <div class="modal-dialog"><div class="modal-content" style="border-radius:14px;overflow:hidden;border:none;">
        <div class="modal-header" style="background:linear-gradient(135deg,#8B1A10,#a52119);border:none;padding:18px 22px;">
            <h5 class="modal-title" style="color:white;font-weight:700;">
                <i class="fas fa-cog mr-2"></i> Configurar información del laboratorio
            </h5>
            <button type="button" class="close" data-dismiss="modal" style="color:white;">&times;</button>
        </div>
        <div class="modal-body" style="padding:24px;">
            <p style="font-size:13px;color:#475569;margin-bottom:18px;">
                Esta información aparece en el encabezado del PDF del reporte.
            </p>
            <form action="<?php echo base_url('reporteservicios/guardar_info'); ?>" method="post">
                <div class="form-group">
                    <label style="font-size:13px;font-weight:600;color:#1e293b;display:block;margin-bottom:6px;">
                        <i class="fas fa-graduation-cap mr-1" style="color:#a52119;"></i> Programa Académico
                    </label>
                    <input type="text" name="programa_academico"
                           value="<?php echo htmlspecialchars($this->session->userdata('programa_academico') ?? ''); ?>"
                           class="form-control"
                           style="border:1.5px solid #e2e8f0;border-radius:8px;padding:9px 13px;font-size:13.5px;"
                ...
                <div class="form-group" style="margin-top:14px;">
                    <label style="font-size:13px;font-weight:600;color:#1e293b;display:block;margin-bottom:6px;">
                        <i class="fas fa-building mr-1" style="color:#a52119;"></i> Edificio
                    </label>
                    <input type="text" name="edificio"
                           value="<?php echo htmlspecialchars($this->session->userdata('edificio') ?? ''); ?>"
                           class="form-control"
                           style="border:1.5px solid #e2e8f0;border-radius:8px;padding:9px 13px;font-size:13.5px;"
                           placeholder="Ej: UD4 UPTx"
                           onfocus="this.style.borderColor='#a52119'"
                           onblur="this.style.borderColor='#e2e8f0'">
                </div>
                <div style="display:flex;gap:10px;margin-top:20px;">
