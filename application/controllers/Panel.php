...
class Panel extends MY_Controller
{
    ...
    public function view($page = 'dashboard')
    {
        ...
        foreach ($tipos as $key => $tipo) {
            $data['tipos'][$key]           = $tipo;
            $data['tipos'][$key]->id_tipos = urlencode($this->idencrypt->encrypt($tipo->id_tipos));
        }
    ...
    // ── Helper: firmantes jefe_laboratorio del lab actual ────
    private function _get_firmantes_inv()
    {
        $laboratorio_id = $this->session->userdata('laboratorio_id');
        return $this->Firmantes_model->getByRolLab('jefe_laboratorio', $laboratorio_id);
    }
    ...
    public function ver_inventario($page = 0)
    {
        ...
        foreach ($equipos as $key => $equipo) {
            ...
            $data['equipos'][$key]->id_equipos = urlencode($this->idencrypt->encrypt($equipo->id_equipos));
        }

        $data['tipo_seleccionado'] = null;
        $data['firmantes_resp']    = $this->_get_firmantes_inv();
    ...
    public function filtrar_inventario()
    {
        ...
        $tipo_id        = !empty($tipo_id_enc) ? (int) $this->idencrypt->decrypt(urldecode($tipo_id_enc)) : 0;
    ...
    {
        $laboratorio_id = $this->session->userdata('laboratorio_id');
        $id_decrypted   = $this->idencrypt->decrypt(urldecode($id));

        if (empty($id_decrypted) || !is_numeric($id_decrypted)) {
            $this->session->set_flashdata('error', 'Equipo no encontrado.');
            redirect('panel/ver_inventario');
            return;
        }
        ...
        if (!$equipo || $equipo->laboratorio_id != $laboratorio_id) {
            $this->session->set_flashdata('error', 'No tienes permiso para ver este equipo.');
            redirect('panel/ver_inventario');
            return;
        }
    ...
    public function editar($id)
    {
        ...
        if (!$equipo || $equipo->laboratorio_id != $laboratorio_id) {
            $this->session->set_flashdata('error', 'No tienes permiso para editar este equipo.');
            redirect('panel/ver_inventario');
            return;
        }
