...
class Inventario extends MY_Controller
{
    ...
    public function registrar()
    {
        ...
        $tipo_id   = (int) $this->idencrypt->decrypt(urldecode($this->input->post('tipo')));
    ...
    public function editar()
    {
        ...
        $id_equipo  = $this->idencrypt->decrypt(urldecode($this->input->post('id_equipos')));
    ...
    {
        if (empty($id)) {
            $this->session->set_flashdata('error', 'ID de equipo no proporcionado.');
            redirect('panel/ver_inventario');
            return;
        }

        $id_real = $this->idencrypt->decrypt(urldecode($id));

        if (empty($id_real) || !is_numeric($id_real) || (int)$id_real <= 0) {
            $this->session->set_flashdata('error', 'ID de equipo inválido.');
            redirect('panel/ver_inventario');
            return;
        }
        ...
        $id_ccompu_obj = $this->Inventario_model->obtener_id_ccompu($id_real);
        if ($id_ccompu_obj !== null && !empty($id_ccompu_obj->id_ccompus)) {
            $this->Inventario_model->eliminar_ccompu($id_ccompu_obj->id_ccompus);
        }

        $eliminado = $this->Inventario_model->eliminar_equipo($id_real);
        ...
        if ($eliminado) {
            ...
            $this->session->set_flashdata('error', 'No se pudo eliminar el equipo.');
        }
