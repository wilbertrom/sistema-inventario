...
class Inventario extends MY_Controller
{
  ...
  public function editar()
  {
    ...
    if ($this->form_validation->run() == FALSE) {
      ...
      // Si el tipo es computadora, recoger los datos adicionales
      if ($this->input->post('tipo') == 'SFNGM2UyUzVYZ3JTT3ZFTGZRMnlaZz09' && !empty($id_ccompus)) {
        ...
        // Llamar al modelo para actualizar los datos en ccompu
        $this->Inventario_model->actualizar_ccompu($data_ccompu_up, $id_ccompus);
      }

      // Llamar al modelo para actualizar los datos en equipos
  ...
  public function eliminar($id)
  {
    ...
    // Verificar que $id_ccompu_obj no sea null y tenga la propiedad id_ccompus
  ...
  public function nuevaMarca()
  {
    ...
    if ($this->form_validation->run() == FALSE) {
      ...
      if ($this->Inventario_model->registrar_marca($data)) {
        // Verificar si viene de la página de editar
        if (!empty($id_equipo)) {
