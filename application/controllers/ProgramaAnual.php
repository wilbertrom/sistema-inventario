...
class ProgramaAnual extends CI_Controller {
    ...
    public function actividades($programa_id)
    {
        ...
        // Cargar firmantes para los dropdowns
        $laboratorio_id = $this->session->userdata('laboratorio_id');
        $data['firmantes_responsable']  = $this->Firmantes_model->getByRolLab('jefe_laboratorio', $laboratorio_id);
        $data['firmantes_revisor']      = $this->Firmantes_model->getByRol('vo_bo');
        $data['firmantes_autorizo']     = $this->Firmantes_model->getByRol('autorizo');
        $data['firmantes_cuatrimestre'] = $this->Firmantes_model->getByRol('cuatrimestre');
